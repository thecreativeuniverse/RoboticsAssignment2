...
class SVOD():
    ...
    def doKmeans(self, scanned_objs):
        self.foundObjects = [(obj, coords) for obj, coords in self.foundObjects if obj not in scanned_objs.keys()]
        for i in range(len(self.itemListNames)):
            ...
            centers = generate_centers(self.itemLists[i], max(min_cluster, 2), min(max_cluster, len(self.itemLists[i])))
            ...
            for coords in centers:
                if tuple(coords)[0] != 300:
                    ...
                    to_keep = min(len(all_items), max(len(centers), 5))
